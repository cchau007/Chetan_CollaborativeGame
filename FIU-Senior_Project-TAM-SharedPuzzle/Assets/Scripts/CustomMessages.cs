...
public class CustomMessages : Singleton<CustomMessages>
{
    ...
    public void SendStageTransform(Vector3 position)
    {
        // If we are connected to a session, broadcast our head info
        if (this.serverConnection != null && this.serverConnection.IsConnected())
        {
    ...
    //create messages for:
    //goal state won
    //maze and ball state since we render other players maze
    //resetting game
    //events
    ...
    public void SendDifficulty(int difficulty)
    {
        ...
        {
            // Create an outgoing network message to contain all the info we want to send
            NetworkOutMessage msg = CreateMessage((byte)TestMessageID.Difficulty);

            msg.Write(difficulty); // send the difficulty message
    ...
    public void SendGoalState(int state)
    {
        // If we are connected to a session
        if (this.serverConnection != null && this.serverConnection.IsConnected())
        {
            // Create an outgoing network message to contain all the info we want to send
            NetworkOutMessage msg = CreateMessage((byte)TestMessageID.GoalState);

            msg.Write(state); // send the state
    ...
    public void SendResetGame()
    {
        ...
        {
            // Create an outgoing network message to contain all the info we want to send
            NetworkOutMessage msg = CreateMessage((byte)TestMessageID.ResetGame);
